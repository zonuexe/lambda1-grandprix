...
fn main() {
    let mut _failures = 0;
    let I: D = { lam(move |x| x.clone()) };
    let K: D = { lam(move |x| { let x = x.clone(); lam(move |y| x.clone()) }) };
    let S: D = { lam(move |x| { let x = x.clone(); lam(move |y| { let x = x.clone(); let y = y.clone(); lam(move |z| x.clone().app(z.clone()).app(y.clone().app(z.clone()))) }) }) };
    let zero: D = { lam(move |f| { lam(move |x| x.clone()) }) };
    let succ: D = { lam(move |n| { let n = n.clone(); lam(move |f| { let f = f.clone(); let n = n.clone(); lam(move |x| f.clone().app(n.clone().app(f.clone()).app(x.clone()))) }) }) };
    let add: D = { lam(move |m| { let m = m.clone(); lam(move |n| { let m = m.clone(); let n = n.clone(); lam(move |f| { let f = f.clone(); let m = m.clone(); let n = n.clone(); lam(move |x| m.clone().app(f.clone()).app(n.clone().app(f.clone()).app(x.clone()))) }) }) }) };
    let _true: D = { lam(move |t| { let t = t.clone(); lam(move |f| t.clone()) }) };
    let _false: D = { lam(move |t| { lam(move |f| f.clone()) }) };
    let _if: D = { lam(move |b| { let b = b.clone(); lam(move |t| { let b = b.clone(); let t = t.clone(); lam(move |e| b.clone().app(t.clone()).app(e.clone())) }) }) };
    let and: D = { lam(move |p| { let p = p.clone(); lam(move |q| p.clone().app(q.clone()).app(p.clone())) }) };
    let not: D = { let _false = _false.clone(); let _true = _true.clone(); lam(move |b| b.clone().app(_false.clone()).app(_true.clone())) };
    _failures += check("assert 1", "1".to_string(), decodeInt(S.clone().app(K.clone()).app(K.clone()).app(encodeInt(1))));
    _failures += check("assert 2", "0".to_string(), decodeInt(zero.clone()));
    _failures += check("assert 3", "3".to_string(), decodeInt(succ.clone().app(encodeInt(2))));
    _failures += check("assert 4", "2".to_string(), decodeInt(add.clone().app(encodeInt(1)).app(encodeInt(1))));
    _failures += check("assert 5", "true".to_string(), decodeBool(and.clone().app(_true.clone()).app(_true.clone())));
    _failures += check("assert 6", "false".to_string(), decodeBool(and.clone().app(_true.clone()).app(_false.clone())));
    _failures += check("assert 7", "false".to_string(), decodeBool(_if.clone().app(_false.clone()).app(_true.clone()).app(_false.clone())));
    _failures += check("assert 8", "true".to_string(), decodeBool(not.clone().app(_false.clone())));
    if _failures > 0 {
