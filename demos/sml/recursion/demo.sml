...
(* --- definitions --- *)
val Z = (Fun (fn f => (app (Fun (fn x => (app f (Fun (fn v => (app (app x x) v)))))) (Fun (fn x => (app f (Fun (fn v => (app (app x x) v)))))))))
val one = (Fun (fn f => (Fun (fn x => (app f x)))))
val mult = (Fun (fn m => (Fun (fn n => (Fun (fn f => (app m (app n f))))))))
...
val isZero = (Fun (fn n => (app (app n (Fun (fn x => v_false))) v_true)))
val fstep = (Fun (fn v_rec => (Fun (fn n => (app (app (app (app isZero n) (Fun (fn u => one))) (Fun (fn u => (app (app mult n) (app v_rec (app pred n)))))) n)))))
val fact = (app Z fstep)

(* --- assertions --- *)
val _ = check "assert 1" ("1") ((decodeInt (app fact (encodeInt 0))))
val _ = check "assert 2" ("1") ((decodeInt (app fact (encodeInt 1))))
val _ = check "assert 3" ("2") ((decodeInt (app fact (encodeInt 2))))
val _ = check "assert 4" ("6") ((decodeInt (app fact (encodeInt 3))))
val _ = check "assert 5" ("120") ((decodeInt (app fact (encodeInt 5))))
