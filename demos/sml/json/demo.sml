...
(* --- definitions --- *)
val pair = (Fun (fn a => (Fun (fn b => (Fun (fn s => (app (app s a) b)))))))
val v_nil = (Fun (fn n => (Fun (fn c => n))))
val cons = (Fun (fn h => (Fun (fn t => (Fun (fn n => (Fun (fn c => (app (app c h) t)))))))))
val v_true = (Fun (fn t => (Fun (fn f => t))))
val v_false = (Fun (fn t => (Fun (fn f => f))))
val snd = (Fun (fn p => (app p v_false)))
val one = (Fun (fn f => (Fun (fn x => (app f x)))))
val pred = (Fun (fn n => (Fun (fn f => (Fun (fn x => (app (app (app n (Fun (fn g => (Fun (fn h => (app h (app g f))))))) (Fun (fn u => x))) (Fun (fn u => u)))))))))
val tint = (Fun (fn k => (app (app pair one) k)))
val step = (Fun (fn p => (app p (Fun (fn k => (Fun (fn l => (app (app pair (app pred k)) (app (app cons (app tint k)) l)))))))))
val range = (Fun (fn n => (app snd (app (app n step) (app (app pair n) v_nil)))))
...
val _ = check "assert 6" ("[1,true]") ((decodeJson (jArr (app (app cons (jInt 1)) (app (app cons (jBool v_true)) v_nil)))))
val _ = check "assert 7" ("{\"k\":1}") ((decodeJson (jObj (app (app cons (app (app pair (jStr "k")) (jInt 1))) v_nil))))
val _ = check "assert 8" ("[1,[2,3]]") ((decodeJson (jArr (app (app cons (jInt 1)) (app (app cons (jArr (app (app cons (jInt 2)) (app (app cons (jInt 3)) v_nil)))) v_nil)))))
val _ = check "assert 9" ("[]") ((decodeJson (jArr (app range (encodeInt 0)))))
val _ = check "assert 10" ("[1]") ((decodeJson (jArr (app range (encodeInt 1)))))
val _ = check "assert 11" ("[1,2,3]") ((decodeJson (jArr (app range (encodeInt 3)))))
