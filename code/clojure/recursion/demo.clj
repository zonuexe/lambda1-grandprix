...
;; --- definitions ---
(def _Z (fn [_f] ((fn [_x] (_f (fn [_v] ((_x _x) _v)))) (fn [_x] (_f (fn [_v] ((_x _x) _v)))))))
(def _one (fn [_f] (fn [_x] (_f _x))))
(def _mult (fn [_m] (fn [_n] (fn [_f] (_m (_n _f))))))
(def _pred (fn [_n] (fn [_f] (fn [_x] (((_n (fn [_g] (fn [_h] (_h (_g _f))))) (fn [_u] _x)) (fn [_u] _u))))))
(def _true (fn [_t] (fn [_f] _t)))
(def _false (fn [_t] (fn [_f] _f)))
(def _isZero (fn [_n] ((_n (fn [_x] _false)) _true)))
(def _fstep (fn [_rec] (fn [_n] ((((_isZero _n) (fn [_u] _one)) (fn [_u] ((_mult _n) (_rec (_pred _n))))) _n))))
(def _fact (_Z _fstep))
