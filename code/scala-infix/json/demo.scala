...
val _pair: D = Fun(_a => Fun(_b => Fun(_s => _s ~ (_a) ~ (_b))))
val _nil: D = Fun(_n => Fun(_c => _n))
val _cons: D = Fun(_h => Fun(_t => Fun(_n => Fun(_c => _c ~ (_h) ~ (_t)))))
val _true: D = Fun(_t => Fun(_f => _t))
val _false: D = Fun(_t => Fun(_f => _f))
val _snd: D = Fun(_p => _p ~ (_false))
val _one: D = Fun(_f => Fun(_x => _f ~ (_x)))
val _pred: D = Fun(_n => Fun(_f => Fun(_x => _n ~ (Fun(_g => Fun(_h => _h ~ (_g ~ (_f))))) ~ (Fun(_u => _x)) ~ (Fun(_u => _u)))))
val _tint: D = Fun(_k => _pair ~ (_one) ~ (_k))
val _step: D = Fun(_p => _p ~ (Fun(_k => Fun(_l => _pair ~ (_pred ~ (_k)) ~ (_cons ~ (_tint ~ (_k)) ~ (_l))))))
val _range: D = Fun(_n => _snd ~ (_n ~ (_step) ~ (_pair ~ (_n) ~ (_nil))))
...
@main def run(): Unit = {
  ...
  check("assert 6", "[1,true]", decodeJson(jArr(_cons ~ (jInt(1)) ~ (_cons ~ (jBool(_true)) ~ (_nil)))))
  check("assert 7", "{\"k\":1}", decodeJson(jObj(_cons ~ (_pair ~ (jStr("k")) ~ (jInt(1))) ~ (_nil))))
  check("assert 8", "[1,[2,3]]", decodeJson(jArr(_cons ~ (jInt(1)) ~ (_cons ~ (jArr(_cons ~ (jInt(2)) ~ (_cons ~ (jInt(3)) ~ (_nil)))) ~ (_nil)))))
  check("assert 9", "[]", decodeJson(jArr(_range ~ (encodeInt(0)))))
  check("assert 10", "[1]", decodeJson(jArr(_range ~ (encodeInt(1)))))
  check("assert 11", "[1,2,3]", decodeJson(jArr(_range ~ (encodeInt(3)))))
