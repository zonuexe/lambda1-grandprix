...
val pair: D = Fun(a => Fun(b => Fun(s => s ~ (a) ~ (b))))
val nil: D = Fun(n => Fun(c => n))
val cons: D = Fun(h => Fun(t => Fun(n => Fun(c => c ~ (h) ~ (t)))))
val _true: D = Fun(t => Fun(f => t))
val _false: D = Fun(t => Fun(f => f))
val snd: D = Fun(p => p ~ (_false))
val one: D = Fun(f => Fun(x => f ~ (x)))
val pred: D = Fun(n => Fun(f => Fun(x => n ~ (Fun(g => Fun(h => h ~ (g ~ (f))))) ~ (Fun(u => x)) ~ (Fun(u => u)))))
val tint: D = Fun(k => pair ~ (one) ~ (k))
val step: D = Fun(p => p ~ (Fun(k => Fun(l => pair ~ (pred ~ (k)) ~ (cons ~ (tint ~ (k)) ~ (l))))))
val range: D = Fun(n => snd ~ (n ~ (step) ~ (pair ~ (n) ~ (nil))))
...
@main def run(): Unit = {
  ...
  check("assert 6", "[1,true]", decodeJson(jArr(cons ~ (jInt(1)) ~ (cons ~ (jBool(_true)) ~ (nil)))))
  check("assert 7", "{\"k\":1}", decodeJson(jObj(cons ~ (pair ~ (jStr("k")) ~ (jInt(1))) ~ (nil))))
  check("assert 8", "[1,[2,3]]", decodeJson(jArr(cons ~ (jInt(1)) ~ (cons ~ (jArr(cons ~ (jInt(2)) ~ (cons ~ (jInt(3)) ~ (nil)))) ~ (nil)))))
  check("assert 9", "[]", decodeJson(jArr(range ~ (encodeInt(0)))))
  check("assert 10", "[1]", decodeJson(jArr(range ~ (encodeInt(1)))))
  check("assert 11", "[1,2,3]", decodeJson(jArr(range ~ (encodeInt(3)))))
