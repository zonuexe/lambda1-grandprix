...
val _S: D = Fun(_x => Fun(_y => Fun(_z => _x ~ (_z) ~ (_y ~ (_z)))))
val _zero: D = Fun(_f => Fun(_x => _x))
val _succ: D = Fun(_n => Fun(_f => Fun(_x => _f ~ (_n ~ (_f) ~ (_x)))))
val _add: D = Fun(_m => Fun(_n => Fun(_f => Fun(_x => _m ~ (_f) ~ (_n ~ (_f) ~ (_x))))))
val _true: D = Fun(_t => Fun(_f => _t))
val _false: D = Fun(_t => Fun(_f => _f))
val _if: D = Fun(_b => Fun(_t => Fun(_e => _b ~ (_t) ~ (_e))))
val _and: D = Fun(_p => Fun(_q => _p ~ (_q) ~ (_p)))
val _not: D = Fun(_b => _b ~ (_false) ~ (_true))

@main def run(): Unit = {
  check("assert 1", "1", decodeInt(_S ~ (_K) ~ (_K) ~ (encodeInt(1))))
  check("assert 2", "0", decodeInt(_zero))
  check("assert 3", "3", decodeInt(_succ ~ (encodeInt(2))))
  check("assert 4", "2", decodeInt(_add ~ (encodeInt(1)) ~ (encodeInt(1))))
  check("assert 5", "true", decodeBool(_and ~ (_true) ~ (_true)))
  check("assert 6", "false", decodeBool(_and ~ (_true) ~ (_false)))
  check("assert 7", "false", decodeBool(_if ~ (_false) ~ (_true) ~ (_false)))
  check("assert 8", "true", decodeBool(_not ~ (_false)))
