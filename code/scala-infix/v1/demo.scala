...
val S: D = Fun(x => Fun(y => Fun(z => x ~ (z) ~ (y ~ (z)))))
val zero: D = Fun(f => Fun(x => x))
val succ: D = Fun(n => Fun(f => Fun(x => f ~ (n ~ (f) ~ (x)))))
val add: D = Fun(m => Fun(n => Fun(f => Fun(x => m ~ (f) ~ (n ~ (f) ~ (x))))))
val _true: D = Fun(t => Fun(f => t))
val _false: D = Fun(t => Fun(f => f))
val _if: D = Fun(b => Fun(t => Fun(e => b ~ (t) ~ (e))))
val and: D = Fun(p => Fun(q => p ~ (q) ~ (p)))
val not: D = Fun(b => b ~ (_false) ~ (_true))

@main def run(): Unit = {
  check("assert 1", "1", decodeInt(S ~ (K) ~ (K) ~ (encodeInt(1))))
  check("assert 2", "0", decodeInt(zero))
  check("assert 3", "3", decodeInt(succ ~ (encodeInt(2))))
  check("assert 4", "2", decodeInt(add ~ (encodeInt(1)) ~ (encodeInt(1))))
  check("assert 5", "true", decodeBool(and ~ (_true) ~ (_true)))
  check("assert 6", "false", decodeBool(and ~ (_true) ~ (_false)))
  check("assert 7", "false", decodeBool(_if ~ (_false) ~ (_true) ~ (_false)))
  check("assert 8", "true", decodeBool(not ~ (_false)))
