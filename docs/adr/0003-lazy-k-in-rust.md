...
## 改訂（2026-07）: zonu-lazyk 0.3 が望ましい性質を実装

`zonu-lazyk` 0.3.0 が上記の望ましい性質を概ね実装した:

- 性質1（項 → チャーチ数の直接評価）: `Program::eval_numeral() -> Result<u64>`（256 上限なし）。
- 性質2/3（非切り詰めの数値列で走査）: `Program::eval_values(input, &DecodeOptions) -> Result<Vec<u64>>`（EOF センチネル・要素数上限・step 上限を `DecodeOptions` で設定）。
- 性質4/6（項を直接入力）: `church_numeral(n) -> Term`、`Program::from_term(Term)`、`Term`/`Comb` 公開。
- 性質5（有界評価）: `Limits { max_steps, max_output_bytes }`、`eval_with`/`run_with`、`Error::StepLimit`/`OutputLimit`。
- 性質7（埋め込み安全な失敗）: `Error`（`Display` / `std::error::Error` 実装）。

これを受け、translator の lazyk backend は **decodeInt / decodeBool を `Program::eval_numeral()` で直接観測**するよう更新した（単項バイトカウントの迂回を撤去。bool は `term 1 0` を 1/0 に落として観測）。バイト出力を経由しないため大きな値（bench の 900）も速い。`run`（byte-stream 一発実行）は後方互換で残り、`lazyk-runner` bin は引き続きこれを使う。

decodeJson は **スカラ（`jInt` / `jBool`）のみ部分対応**（int/bool と同じく `eval_numeral` で observe）。array / string / object / null や `range` 由来の計算リストは Scott リスト走査（fixpoint 相当）が要り純 SKI で非現実的なため skip。性質2（`eval_values`）を使えば、将来フラットな int 配列まで広げられる余地がある。
