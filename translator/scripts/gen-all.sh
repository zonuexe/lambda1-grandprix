#!/usr/bin/env bash
# 全コーパス × 全言語の生成コードを、聴衆が目視できるよう code/<言語>/<コーパス>/ に出力する。
#
#   translator/scripts/gen-all.sh            # 既定: リポジトリ直下の code/ へ
...
# 索引 README（言語 × コーパスの表）
{
  ...
  echo "\`code/<言語>/<コーパス>/\` に自己完結で配置（demo.<ext> ＋ 分離ライブラリ ＋ source.lam）。"
